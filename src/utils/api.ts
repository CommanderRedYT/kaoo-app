// react-native
import {OrderRequest, OrderResponse} from "../models/kaoo";
...
export async function getGoods(shopId: string) {
    ...
    return await request(`http://order.huaqiaobang.com/index.php?ctrl=shop&action=jmGetGoodsType&id=${shopId}`, {
        method: "GET",
        headers: {
...
export async function getOrderHistory(shopId: string, table_num: string) {
    ...
    return await request(`http://order.huaqiaobang.com/index.php?ctrl=order&action=jmOrderHistory&shopid=${shopId}&table_num=${table_num}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
        },
...
export async function makeOrder(order: OrderRequest): Promise<OrderResponse | void> {
